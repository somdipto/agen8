...
export function generateWorkflowCode(
  ...
  const paramDeclarations = parameters.map((p) => `${p.name}: ${p.type}`).join(', ');

  // Start function
  codeLines.push(`export async function ${functionName}(${paramDeclarations}): ${returnType} {`);
  codeLines.push(`  "use workflow";`);
  codeLines.push(``);

  // Generate code for each node in the workflow
  function generateNodeCode(nodeId: string, indent: string = '  '): string[] {
    ...
    switch (node.data.type) {
      ...
      case 'action':
        ...
        if (actionType === 'Send Email' || node.data.label.toLowerCase().includes('email')) {
          ...
          lines.push(`${indent}  subject: input.subject as string || 'Notification',`);
          ...
          lines.push(`${indent}  title: input.title as string || 'New Ticket',`);
          lines.push(`${indent}  description: input.description as string || '',`);
          lines.push(`${indent}});`);
        } else if (
          actionType === 'Database Query' ||
          node.data.label.toLowerCase().includes('database')
        ) {
          imports.add("import { executeQuery, insertData, queryData } from './integrations';");
          lines.push(`${indent}const ${varName} = await queryData('your_table', {});`);
          ...
          lines.push(`${indent}  url: '${endpoint || 'https://api.example.com/endpoint'}',`);
          lines.push(`${indent}  method: 'POST',`);
          lines.push(`${indent}  body: input,`);
          lines.push(`${indent}});`);
        } else {
          lines.push(`${indent}const ${varName} = { status: 'success', data: input };`);
        }
        break;
